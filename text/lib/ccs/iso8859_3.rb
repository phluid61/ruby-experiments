...
# ISO-8859-3
#
CCS::ISO8859_3 = CCS.new('ISO-8859-3', 0, 255) do
  def valid? cp
    CCS::ISO8859_3_FORWARD[cp]
  end

  def to_ucs cp
    CCS::ISO8859_3_FORWARD[cp] or raise CES::EncodingError, "invalid codepoint #{render_codepoint cp}"
  end

  def from_ucs cp
    CCS::ISO8859_3_REVERSE[cp] or raise CES::EncodingError, "invalid codepoint #{render_codepoint cp}"
  end
...
##
# ISO-8859-3, strict mode (no control characters)
#
CCS::ISO8859_3_Strict = CCS.new('ISO-8859-3 (strict)', 0, 255) do
  def valid? cp
    ((cp >= 0x20 && cp <= 0x7E) || (cp >= 0xA0 && cp <= 0xFF)) && CCS::ISO8859_3_FORWARD[cp]
  end
...
CCS::ISO8859_3_FORWARD = [
  ...
  0x00A0,0x0126,0x02D8,0x00A3,0x00A4,nil,   0x0124,0x00A7,0x00A8,0x0130,0x015E,0x011E,0x0134,0x00AD,nil,   0x017B, # A_
  0x00B0,0x0127,0x00B2,0x00B3,0x00B4,0x00B5,0x0125,0x00B7,0x00B8,0x0131,0x015F,0x011F,0x0135,0x00BD,nil,   0x017C, # B_
  0x00C0,0x00C1,0x00C2,nil,   0x00C4,0x010A,0x0108,0x00C7,0x00C8,0x00C9,0x00CA,0x00CB,0x00CC,0x00CD,0x00CE,0x00CF, # C_
  nil,   0x00D1,0x00D2,0x00D3,0x00D4,0x0120,0x00D6,0x00D7,0x011C,0x00D9,0x00DA,0x00DB,0x00DC,0x016C,0x015C,0x00DF, # D_
  0x00E0,0x00E1,0x00E2,nil,   0x00E4,0x010B,0x0109,0x00E7,0x00E8,0x00E9,0x00EA,0x00EB,0x00EC,0x00ED,0x00EE,0x00EF, # E_
  nil,   0x00F1,0x00F2,0x00F3,0x00F4,0x0121,0x00F6,0x00F7,0x011D,0x00F9,0x00FA,0x00FB,0x00FC,0x016D,0x015D,0x02D9, # F_
  #   _0     _1     _2     _3     _4     _5     _6     _7     _8     _9     _A     _B     _C     _D     _E     _F
].freeze
CCS::ISO8859_3_REVERSE = Hash[CCS::ISO8859_3_FORWARD.each_with_index.map {|e, i| [e, i] }.reject{|a, b| a.nil? }]
