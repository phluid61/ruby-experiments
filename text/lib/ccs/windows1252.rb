...
# Windows-1252 / Latin1.
#
CCS::WINDOWS1252 = CCS.new(0, 255) do
  ...
  REVERSE = Hash[FORWARD.each_with_index {|e, i| [i, e] }]
  # rubocop:enable Metrics/LineLength, Layout/SpaceAfterComma, Style/TrailingCommaInLiteral

  def to_ucs cp
    FORWARD[cp] or raise "invalid codepoint #{render_codepoint cp}"
  end

  def from_ucs cp
    REVERSE[cp] or raise "invalid codepoint #{render_codepoint cp}"
  end
end
