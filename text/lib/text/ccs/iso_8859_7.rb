...
table = [
  ...
  0x00A0,0x2018,0x2019,0x00A3,0x20AC,0x20AF,0x00A6,0x00A7,0x00A8,0x00A9,0x037A,0x00AB,0x00AC,0x00AD,0x00AE,0x2015, # A_
  0x00B0,0x00B1,0x00B2,0x00B3,0x0384,0x0385,0x0386,0x00B7,0x0388,0x0389,0x038A,0x00BB,0x038C,0x00BD,0x038E,0x038F, # B_
  0x0390,0x0391,0x0392,0x0393,0x0394,0x0395,0x0396,0x0397,0x0398,0x0399,0x039A,0x039B,0x039C,0x039D,0x039E,0x039F, # C_
  0x03A0,0x03A1,0x00D2,0x03A3,0x03A4,0x03A5,0x03A6,0x03A7,0x03A8,0x03A9,0x03AA,0x03AB,0x03AC,0x03AD,0x03AE,0x03AF, # D_
  0x03B0,0x03B1,0x03B2,0x03B3,0x03B4,0x03B5,0x03B6,0x03B7,0x03B8,0x03B9,0x03BA,0x03BB,0x03BC,0x03BD,0x03BE,0x03BF, # E_
  0x03C0,0x03C1,0x03C2,0x03C3,0x03C4,0x03C5,0x03C6,0x03C7,0x03C8,0x03C9,0x03CA,0x03CB,0x03CC,0x03CD,0x03CE,0x00FF, # F_
  #   _0     _1     _2     _3     _4     _5     _6     _7     _8     _9     _A     _B     _C     _D     _E     _F
].freeze
...
#
CCS::ISO_8859_7 = TableCCS.new('ISO-8859-7', 0, 255, table) do
...
CCS::LATIN_GREEK   = CCS::ISO_8859_7
CCS::CP813         = CCS::ISO_8859_7
CCS::WINDOWS_28597 = CCS::ISO_8859_7

##
# ISO-8859-7, strict mode (no control characters)
#
CCS::ISO_8859_7_Strict = TableCCS.new('ISO-8859-7 (strict)', 0x20, 0xD2, table) do
  def valid? cp
    (cp >= 0x20 && cp <= 0x7E) || ((cp >= 0xA0 && cp <= 0xFE) && cp != 0xAE && cp != 0xD2)
  end
